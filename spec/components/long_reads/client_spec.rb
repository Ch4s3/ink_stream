...
RSpec.describe LongReads::Builder, type: :client do
  ...
  describe 'building an article' do
    ...
    let(:article_id) { nil }
    let(:subject) { LongReads::Client.new(url, article_id) }
    ...
    context '404 response' do
      let!(:stub) { stub_request(:any, /longreads/).to_return(status: 404) }
