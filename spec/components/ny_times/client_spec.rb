...
RSpec.describe NyTimes::Builder, type: :client do
  ...
  describe 'building an article' do
    ...
    let(:url) { 'https://www.nytimes.com//2017/10/06/business/the-office-gets-remade-again.html' }
    let(:article_id) { nil }
    let(:subject) { NyTimes::Client.new(url, article_id) }

    context 'good response' do
      let!(:stub) { stub_request(:any, /nytimes/).to_return(body: response_body) }
    ...
    context '404 response' do
      let!(:stub) { stub_request(:any, /nytimes/).to_return(status: 404) }
